...
import { Lock as LockIcon, Save, Calendar, Video, User, Check, Loader2, ArrowLeft } from 'lucide-react';
...
export default function SessionAdminPage() {
    ...
    const fetchConfig = () => {
        setFetching(true);
        try {
            const savedConfig = localStorage.getItem('liveSessionConfig');
            if (savedConfig) {
    ...
    const handleSave = () => {
        setLoading(true);
        try {
            localStorage.setItem('liveSessionConfig', JSON.stringify(sessionData));
            setSuccess(true);
            ...
            <header style={{ padding: '1.5rem 2rem', borderBottom: '1px solid var(--border-color)', background: 'var(--bg-secondary)', position: 'sticky', top: 0, zIndex: 10 }}>
                <div style={{ maxWidth: '1000px', margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        <button onClick={() => navigate('/track')} style={{ background: 'none', border: 'none', color: 'var(--text-secondary)', cursor: 'pointer' }}>
                            <ArrowLeft size={24} />
                        </button>
